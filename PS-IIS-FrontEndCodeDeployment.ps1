# -----------------------------------------------------------------------------
# File: PS-IIS-FrontEndCodeDeployment.ps1
# Description: This script deploys front-end code into production.
# Author: Eric Post
# Date: Apr 28, 2023
# -----------------------------------------------------------------------------
# -> Sanitized for GitHub using the MIT license. 
#
# Notes:
#
# This script transfers front-end code from the deployment repository to WebServer01, which serves as the primary DFS replication server. 
# The code then propagates out to all the IIS production servers within 5 minutes.
#
# This script comes with a GUI. You need to provide the instance (customer code), code version, and the TargetProcess URL related to the deployment.
#
# Next, it stops the IIS site/app pools, deletes the contents of the destination folder (excluding specific exceptions), and proceeds to deploy the new code to that directory.
#
# Following this, it restarts the IIS site/app pools and sends an email to changeManagementSystem@company.com, providing details about the deployment, including the changes made, 
# the responsible person, and the deployment location.
...
$enter.Add_Click({
    ...
    # EXCLUDING the qb folder, Web.config, and newrelic.config.
